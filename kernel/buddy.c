...
void bit_recv(char *array, int index) {
  index /= 2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_recv(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    bit_recv(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_recv(bd_sizes[k].alloc, bi);  // free p at size k
